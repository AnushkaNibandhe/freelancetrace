                        ...
                        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                          ...
                          <DialogContent className="sm:max-w-lg">
                            ...
                            <div className="space-y-4 py-4">
                              ...
                              {bidAmount && selectedProject?.budget_max && parseFloat(bidAmount) > selectedProject.budget_max && (
                                <Alert className="border-yellow-300 bg-yellow-50">
                                  <AlertTriangle className="h-4 w-4 text-yellow-600" />
                                  <AlertDescription className="text-yellow-800">
                                    Your bid is {Math.round(((parseFloat(bidAmount) - selectedProject.budget_max) / selectedProject.budget_max) * 100)}% above the project budget (₹{selectedProject.budget_max.toLocaleString()})
                                  </AlertDescription>
                                </Alert>
                              )}
                              {bidDuration && selectedProject?.duration_days && parseInt(bidDuration) > selectedProject.duration_days && (
                                <Alert className="border-yellow-300 bg-yellow-50">
                                  <AlertTriangle className="h-4 w-4 text-yellow-600" />
                                  <AlertDescription className="text-yellow-800">
                                    Your timeline is {parseInt(bidDuration) - selectedProject.duration_days} days over the project duration ({selectedProject.duration_days} days)
                                  </AlertDescription>
                                </Alert>
                              )}
                              <div className="space-y-2">
