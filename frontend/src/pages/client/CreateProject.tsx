...
import { useCreateProject, useUploadSRSToProject } from '@/hooks/useProjects';
...
const CreateProject: React.FC = () => {
  ...
  const [srsResult, setSrsResult] = useState<{ count: number; pipeline: string } | null>(null);
  ...
  const isMdFile = (file: File | null): boolean =>
    file !== null && file.name.toLowerCase().endsWith('.md');
  ...
  const handleSubmit = async () => {
    try {
      ...
      const projectId = String((newProject as any)?.id || '');

      if (formData.srsFile && projectId) {
        setSrsAnalyzing(true);
        try {
          if (isMdFile(formData.srsFile)) {
            // Step 2a: .md file — parse requirements using multipart form
            const formPayload = new FormData();
            formPayload.append('file', formData.srsFile);
            const parseRes = await apiFetch(`/projects/${projectId}/parse-requirements`, {
              method: 'POST',
              body: formPayload,
            });
            if (!parseRes.ok) {
              throw new Error(`parse-requirements failed: ${parseRes.status}`);
            }
            const parseResult = await parseRes.json();
            setSrsResult({ count: parseResult.requirements_extracted, pipeline: 'md' });

            // Step 2b: auto-generate the SRS PDF from the parsed requirements
            const genRes = await apiFetch(`/projects/${projectId}/generate-srs`, {
              method: 'POST',
              body: JSON.stringify({}),
            });
            // Generation failure is non-fatal — project and requirements are already saved
            if (!genRes.ok) {
              console.warn('SRS generation failed (non-fatal):', await genRes.text());
            }
          } else {
            // PDF/DOC/DOCX — use existing upload path (LLM extraction)
            const result = await uploadSRSToProject.mutateAsync({ projectId, file: formData.srsFile });
            setSrsResult({ count: result.requirements_extracted, pipeline: result.pipeline });
          }
        } catch (err) {
          console.error('Requirements analysis failed (project still created):', err);
        } finally {
          setSrsAnalyzing(false);
        }
      }

      // Step 3: Navigate to the project requirements tab
      navigate(`/client/projects/${projectId}?tab=requirements`);
    } catch {
      // createProject.onError already shows a toast
    }
  };
  ...
  const isSubmitting = createProject.isPending || uploadSRSToProject.isPending || srsAnalyzing;
        ...
        {step === 3 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Upload a requirements document or add requirements manually</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                  ...
                  {formData.srsFile ? (
                    ...
                    <label htmlFor="srs-upload" className="cursor-pointer">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Drag and drop or click to upload (PDF, DOC, DOCX, MD)
                      </p>
                    </label>
                  )}
                </div>
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-4">
                ...
                <div className="p-4 rounded-lg bg-muted">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {formData.srsFile
                      ? `Requirements document: ${formData.srsFile.name}`
              ...
              {srsResult && (
                <p className="text-sm text-center text-green-600 font-medium">
                  ✅ Extracted {srsResult.count} requirements via{' '}
                  {srsResult.pipeline === 'md'
                    ? 'Markdown parser'
                    : srsResult.pipeline === 'gemini'
                    ? 'Gemini AI'
                    : srsResult.pipeline}
                </p>
              )}
